...
#define PLUGIN "DoD Unlimited Grenades"
#define VERSION "1.0"
...
public plugin_init(){
   ...
   g_nades = register_cvar("dod_grenades","0")
   register_event("ResetHUD","give_grenades","be")
}

public give_grenades(id){
	if(get_pcvar_num(g_nades)){
		set_task(0.1,"give_nades",id)
...
public give_nades(id){
	if(get_user_team(id) == AXIS){
		dod_set_user_ammo(id,DODW_STICKGRENADE,999)
		return PLUGIN_HANDLED
	}
	else if(get_user_team(id) == ALLIES){
		if(dod_get_map_info(MI_ALLIES_TEAM) == 1){
			dod_set_user_ammo(id,DODW_MILLS_BOMB,999)
			return PLUGIN_HANDLED
		}
		else if(dod_get_map_info(MI_ALLIES_TEAM) == 0){
			dod_set_user_ammo(id,DODW_HANDGRENADE,999)
			return PLUGIN_HANDLED
		}
        }
        return PLUGIN_HANDLED
