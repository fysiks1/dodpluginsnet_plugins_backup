...
new cvar_medic_strip,cvar_medic_distance,cvar_medic_menu,cvar_medic_minhp;
...
new colt_ammo, luger_ammo, webley_ammo, grenade_ammo;
new cvar_axis_medic_limit,cvar_allied_medic_limit,cvar_medic_menu_time,cvar_medic_autocall_health;
new cvar_medic_autocall,cvar_medic_callmenu;
//global booleans
...
//custom weapon support for stats logging
new wpnid_medkit,wpnid_syringe,wpnid_heal;
...
public plugin_init() 
{
	...
	cvar_medic_strip = register_cvar("dod_enable_medic_strip", "1"); //enables/disables medic ability to pickup primary weapons (1 = strip and remove)
	...
	cvar_medic_callmenu = register_cvar("dod_enable_medic_call_menu", "0"); //enables/disables medic menu to quickly allow soldiers to call for em adic
	cvar_medic_autocall = register_cvar("dod_enable_auto_call", "0"); //enables/disables medic automatic call (if below certain value)
	cvar_medic_autocall_health = register_cvar("dod_medic_auto_call_health", "20"); //auto call a medic if players health falls below this value
	...
	colt_ammo = register_cvar("dod_medic_colt_ammo", "7"); //amount of pistol bullets to give the US medic when touching a dropped weapon
	luger_ammo = register_cvar("dod_medic_luger_ammo", "8"); //amount of pistol bullets to give the Axis medic when touching a dropped weapon
	webley_ammo = register_cvar("dod_medic_webley_ammo", "6"); //amount of pistol bullets to give the British medic when touching a dropped weapon
	...
	wpnid_medkit = custom_weapon_add("MedKit",0,"MedKit");
	wpnid_syringe = custom_weapon_add("MedSyringe",0,"MedSyringe");
	wpnid_heal = custom_weapon_add("healing",0,"healing");
...
public client_damage(attacker, victim, damage, wpnindex, hitplace, TA)
{
	if(plugin_enabled)
	{
		if(medic_mod_enabled && get_pcvar_num(cvar_medic_autocall) && !is_user_bot(victim))
		{
			new victim_health  = get_user_health(victim);
			new dmg_autocall = get_pcvar_num(cvar_medic_autocall_health);
			if(victim_health <= dmg_autocall) 
			{
				call_medic(victim);
			}
			if(get_pcvar_num(cvar_medic_callmenu) && !stop_medic_menu[victim] ) 
			{
				if(damage >= dmg_autocall) set_task(3.0,"ShowMedic_Call",victim);
			}
		}
	}
	return PLUGIN_CONTINUE;
...
public go_medic(id)
{
	...
	{	
		if(medic_mod_enabled)
		{
			...
			if(class == DODC_MG34 || class == DODC_MG42 || class == DODC_30CAL || class == DODC_SNIPER || class == DODC_SCHARFSCHUTZE || class == DODC_SCOPED_FG42 || class == DODC_PIAT || class == DODC_PANZERJAGER || class == DODC_MARKSMAN) return PLUGIN_HANDLED;
...
public PlayerPreThink(id)
{
	...
	{	
		if(medic_mod_enabled)
		{
			if(!is_user_alive(id) || is_user_bot(id)) return FMRES_HANDLED; 
	...
	{	
		...
		if(strfind(classname,"ammo_generic",0)>= 0  && ismedic[ptd]) 
		{
			...
			if(team == ALLIES && !britishally && owner != ptd && strfind(classname,"american",0)>=0) //if ally
			{
				...
				remove_entity(ptr);
				// set pistol ammo
				dod_set_user_ammo(ptd,DODW_COLT, ammo + coltAmmo);
				// Update user's HUD
				message_begin(MSG_ONE,msgAmmoX,{0,0,0},ptd);
				write_byte(4);
				write_byte(ammo + coltAmmo);
				message_end();
			...
			else if (team == ALLIES && britishally && owner != ptd && strfind(classname,"british",0)>=0) //if british
			{
				...
				remove_entity(ptr);
				// set pistol ammo
				dod_set_user_ammo(ptd,DODW_WEBLEY, ammo + webleyAmmo);
				// Update user's HUD
				message_begin(MSG_ONE,msgAmmoX,{0,0,0},ptd);
				write_byte(4);
				write_byte(ammo + webleyAmmo);
				message_end();
			}
			else if(team == AXIS && owner != ptd && strfind(classname,"german",0)>=0) //if axis
			{
				...
				remove_entity(ptr);
				// set pistol ammo
				dod_set_user_ammo(ptd,DODW_LUGER, ammo + lugerAmmo);
				// Update user's HUD
				message_begin(MSG_ONE,msgAmmoX,{0,0,0},ptd);
				write_byte(4);
				write_byte(ammo + lugerAmmo);
				message_end();
			}
			if((myWeapon == DODW_HANDGRENADE_EX || myWeapon == DODW_STICKGRENADE_EX))
			{
				if (team == ALLIES)
				{
					dod_set_user_ammo(ptd,DODW_HANDGRENADE,ammo + 1);
					// Update user's HUD
					message_begin(MSG_ONE,msgAmmoX,{0,0,0},ptd);
					write_byte(9);
					write_byte(ammo + 1);
				...
				{
					dod_set_user_ammo(ptd,DODW_STICKGRENADE,ammo + 1);
					// Update user's HUD
					message_begin(MSG_ONE,msgAmmoX,{0,0,0},ptd);
					write_byte(9);
					write_byte(ammo + 1);
		...
		if((strfind(classname,"weaponbox",0)>=0 || strfind(classname,"weapon_",0)>=0) && ismedic[ptd])
		{
			...
			if(get_pcvar_num(cvar_medic_strip))
			{
				
				client_print(ptd,print_center,"%L",ptd,"MEDIC_PRIMARY_DISABLED");
				remove_entity(ptr);
				return PLUGIN_HANDLED;
			}
			if((myWeapon == DODW_HANDGRENADE_EX || myWeapon == DODW_STICKGRENADE_EX))
...
public boost_duration(ida[])
{
	...
	//custom weapon support
	if(is_user_connected(owner) && is_user_connected(id)) custom_weapon_dmg(wpnid_syringe,owner,id,boostdmg,random_num(2,7));
...
public throw_medkit(id)
{
	...
	//custom weapon support
	custom_weapon_shot(wpnid_medkit,id);
	...
	if (get_pcvar_num(cvar_medic_pstats)== 1)
	{
		//custom weapon support
		custom_weapon_shot(wpnid_medkit,id);
	}
...
public throw_syringe(id)
{
	...
	//custom weapon support
	custom_weapon_shot(wpnid_syringe,id);
			...
			for (p=0;p<33;p++) //players
			{
				...
				if(is_user_alive(p) && p != m)
				{
					...
					if(distance < freehealth_distance && mteam == pteam)
					{
						...
						{
							//custom weapon support
							custom_weapon_shot(wpnid_heal,m);
